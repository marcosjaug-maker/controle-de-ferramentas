...
// ─── Constants ────────────────────────────────────────────────────────────────
const CATEGORIES = ['Electrical', 'Heavy', 'Measuring', 'Hand Tools', 'Hydraulic', 'Other']
const USER_KEY = 'ft_username'

const STATUS = {
  available: { label: 'Available', color: '#22c55e', bg: '#052e16' },
  out:       { label: 'On Site',   color: '#f97316', bg: '#1c0a00' },
  overdue:   { label: 'Overdue',   color: '#ef4444', bg: '#200000' },
}
  ...
  return (
    <div style={{
      ...
      <div style={{ marginBottom: 40, textAlign: 'center', animation: 'slideUp 0.3s ease' }}>
        ...
        <h1 style={{ fontFamily: 'Syne, sans-serif', fontSize: 26, fontWeight: 800, letterSpacing: '-0.03em', marginBottom: 6, color: '#fff' }}>
          ToolTrack
        </h1>
        <p style={{ color: '#555', fontSize: 12 }}>Tool tracking for job sites</p>
      </div>
      ...
      <div style={{
        ...
        <p style={{ fontSize: 13, color: '#888', marginBottom: 24, lineHeight: 1.6 }}>
          Enter your name to get started.<br />
          <span style={{ color: '#444', fontSize: 11 }}>Your actions will be recorded in the system.</span>
        </p>
        ...
        <Btn full onClick={handleSubmit}>Enter App →</Btn>
      </div>

      <p style={{ color: '#333', fontSize: 11, marginTop: 28, textAlign: 'center' }}>
        No password required — just your name
      </p>
    </div>
  )
}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        setOnline(false)
        showToast('No database connection. Check your internet.', 'error')
      } finally {
        setLoading(false)
      }
    }
  ...
  const handleSend = async () => {
    ...
    setSaving(false)
    if (error) { showToast('Error saving. Please try again.', 'error'); return }
    closeModal(); showToast('Checkout recorded!')
  }
  ...
  const handleReturn = async (recordId) => {
    ...
    const { error } = await supabase.from('records')
      ...
      .eq('id', recordId)
    setSaving(false)
    if (error) { showToast('Error recording return.', 'error'); return }
    closeModal(); showToast('Return recorded!')
  }
  ...
  const handleAddTool = async () => {
    ...
    setSaving(false)
    if (error?.code === '23505') { showToast('Serial number already exists.', 'error'); return }
    if (error) { showToast('Error saving.', 'error'); return }
    closeModal(); showToast(`"${form.toolName.trim()}" added!`)
  }
  ...
  const handleAddSite = async () => {
    ...
    setSaving(false)
    if (error) { showToast('Error saving.', 'error'); return }
    closeModal(); showToast(`Site "${form.siteName.trim()}" added!`)
  }
  ...
  const handleExport = () => {
    try {
      ...
      const histData = records.map(r => {
        ...
        return {
          'Tool': tool?.name || '', 'Serial': tool?.serial || '', 'Category': tool?.category || '',
          'Site': site?.name || '', 'Address': site?.address || '',
          'Responsible': r.responsible,
          'Checkout Date': r.delivery_date, 'Expected Return': r.return_date,
          'Actual Return': r.actual_return || '',
          'Status': r.actual_return ? 'Returned' : r.return_date < today ? 'Overdue' : 'On Site',
          'Checked Out By': r.created_by || '',
          'Returned By': r.returned_by || '',
          'Notes': r.notes || '',
        }
      })
      const ws1 = XLSX.utils.json_to_sheet(histData.length ? histData : [{ 'No records': '' }])
      ...
      const ws2 = XLSX.utils.json_to_sheet(tools.map(t => {
        ...
        return {
          'Name': t.name, 'Serial': t.serial, 'Category': t.category,
          'Status': STATUS[s].label,
          'Current Site': sites.find(s => s.id === rec?.site_id)?.name || '—',
          'Responsible': rec?.responsible || '—',
          'Expected Return': rec?.return_date || '—',
        }
      }))
      XLSX.utils.book_append_sheet(wb, ws2, 'Tools')

      const ws3 = XLSX.utils.json_to_sheet(sites.map(s => {
        const active = records.filter(r => r.site_id === s.id && !r.actual_return)
        return {
          'Site': s.name, 'Address': s.address, 'Active Tools': active.length,
          'Tools': active.map(r => tools.find(t => t.id === r.tool_id)?.name).join(', ') || '—',
        }
      }))
      XLSX.utils.book_append_sheet(wb, ws3, 'Sites')

      XLSX.writeFile(wb, `ToolTrack_${today}.xlsx`)
      showToast('Spreadsheet exported! ✅')
    } catch { showToast('Export error. Try again.', 'error') }
  }
  ...
  return (
    <div style={{ minHeight: '100dvh', background: '#0a0a0a', color: '#fff', fontFamily: "'DM Mono', monospace", paddingBottom: 84 }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ fontFamily: 'Syne, sans-serif', fontWeight: 800, fontSize: 14 }}>
            ToolTrack <SyncDot online={online} />
          </span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <Btn small variant="green" onClick={handleExport}>↓ Export</Btn>
      ...
      <div style={{ padding: '20px 16px', maxWidth: 640, margin: '0 auto' }}>
          ...
          <div style={{ animation: 'slideUp 0.2s ease' }}>
            <div style={{ marginBottom: 22 }}>
              <h1 style={{ fontFamily: 'Syne, sans-serif', fontSize: 24, fontWeight: 800 }}>
                Hey, {currentUser.split(' ')[0]} 👋
              </h1>
              <p style={{ color: '#555', fontSize: 12, marginTop: 3 }}>Real-time tool status</p>
            </div>

            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 20 }}>
              {[
                { label: 'Total Inventory', value: stats.total,     color: '#aaa',    icon: '🔩' },
                { label: 'Available',       value: stats.available, color: '#22c55e', icon: '✅' },
                { label: 'On Site',         value: stats.out,       color: '#f97316', icon: '🏗️' },
                { label: 'Overdue Returns', value: stats.overdue,   color: '#ef4444', icon: '⚠️' },
              ].map(s => (
            ...
            {stats.overdue > 0 && (
              <div style={{ background: '#160000', border: '1px solid #ef444430', borderRadius: 12, padding: '14px 16px', marginBottom: 18 }}>
                <div style={{ color: '#ef4444', fontWeight: 700, fontSize: 11, letterSpacing: '0.06em', marginBottom: 10 }}>⚠️ OVERDUE RETURNS</div>
                {tools.filter(t => getToolStatus(t.id) === 'overdue').map(t => {
                  ...
                  return (
                    <div key={t.id} style={{ fontSize: 12, color: '#fca5a5', marginBottom: 6, lineHeight: 1.6 }}>
                      ...
                      <span style={{ color: '#888' }}>{site?.name} · {r?.responsible} · Due: {r?.return_date}</span>
                    </div>
                  )
                })}
              </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              <Btn full onClick={() => setModal('send')}>+ Check Out a Tool</Btn>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                <Btn full variant="blue" onClick={() => setModal('addTool')}>+ New Tool</Btn>
                <Btn full variant="blue" onClick={() => setModal('addSite')}>+ New Site</Btn>
              </div>
            </div>
          </div>
        )}
        ...
        {tab === 'tools' && (
          <div style={{ animation: 'slideUp 0.2s ease' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 18 }}>
              <div>
                <h1 style={{ fontFamily: 'Syne, sans-serif', fontSize: 22, fontWeight: 800 }}>Tools</h1>
                <p style={{ color: '#555', fontSize: 11, marginTop: 1 }}>{tools.length} registered</p>
              </div>
              <Btn small variant="blue" onClick={() => setModal('addTool')}>+ New</Btn>
            </div>
            ...
            <div style={{ display: 'flex', gap: 6, marginBottom: 14, overflowX: 'auto', paddingBottom: 4 }}>
              {['all', 'available', 'out', 'overdue'].map(f => (
                ...
                }}>{f === 'all' ? 'All' : STATUS[f]?.label}</button>
              ))}
            </div>

            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {filteredTools.length === 0 && (
                <div style={{ color: '#333', fontSize: 13, padding: '24px 0', textAlign: 'center' }}>No tools found.</div>
              )}
                    ...
                    {rec && (
                      <div style={{ background: '#161616', borderRadius: 8, padding: '9px 11px', display: 'flex', alignItems: 'center', gap: 8 }}>
                        <div style={{ flex: 1, fontSize: 11, color: '#777', lineHeight: 1.7 }}>
                          📍 {site?.name}<br />
                          👤 {rec.responsible} · Due: <span style={{ color: rec.return_date < today ? '#ef4444' : '#888' }}>{rec.return_date}</span><br />
                          <span style={{ color: '#444' }}>Checked out by: {rec.created_by}</span>
                        </div>
                        <Btn small variant="ghost" onClick={() => { setForm({ recordId: rec.id, toolName: tool.name }); setModal('return') }}>Return</Btn>
                      </div>
                    )}
                  </div>
                )
        ...
        {tab === 'sites' && (
          <div style={{ animation: 'slideUp 0.2s ease' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 18 }}>
              <div>
                <h1 style={{ fontFamily: 'Syne, sans-serif', fontSize: 22, fontWeight: 800 }}>Job Sites</h1>
                <p style={{ color: '#555', fontSize: 11, marginTop: 1 }}>{sites.length} registered</p>
              </div>
              <Btn small variant="blue" onClick={() => setModal('addSite')}>+ New</Btn>
            </div>
              ...
              {sites.map(site => {
                ...
                return (
                  <div key={site.id} style={{ background: '#111', border: '1px solid #1a1a1a', borderRadius: 14, padding: '16px' }}>
                    ...
                    {active.length === 0
                      ? <div style={{ fontSize: 12, color: '#2a2a2a', fontStyle: 'italic' }}>No tools currently assigned.</div>
                      : active.map(r => {
                          const tool = tools.find(t => t.id === r.tool_id)
                            ...
                            <div key={r.id} style={{ display: 'flex', alignItems: 'center', gap: 10, background: '#161616', borderRadius: 9, padding: '10px 12px', marginBottom: 6 }}>
                              <div style={{ flex: 1, minWidth: 0 }}>
                                ...
                                <div style={{ fontSize: 11, color: '#555' }}>
                                  {r.responsible} · Due: <span style={{ color: overdue ? '#ef4444' : '#666' }}>{r.return_date}</span>
                                </div>
                              </div>
                              <Badge status={overdue ? 'overdue' : 'out'} />
                              <Btn small variant="ghost" onClick={() => { setForm({ recordId: r.id, toolName: tool?.name }); setModal('return') }}>Return</Btn>
                            </div>
                          )
                        })
                    }
        ...
        {tab === 'history' && (
          <div style={{ animation: 'slideUp 0.2s ease' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 18 }}>
              <div>
                <h1 style={{ fontFamily: 'Syne, sans-serif', fontSize: 22, fontWeight: 800 }}>History</h1>
                <p style={{ color: '#555', fontSize: 11, marginTop: 1 }}>{records.length} records</p>
              </div>
              <Btn small variant="green" onClick={handleExport}>↓ Export</Btn>
            </div>
                ...
                return (
                  <div key={r.id} style={{ background: '#111', border: '1px solid #1a1a1a', borderRadius: 12, padding: '13px 15px' }}>
                    ...
                    <div style={{ fontSize: 11, color: '#666', lineHeight: 1.9 }}>
                      🏗️ {site?.name}<br />
                      👤 {r.responsible}<br />
                      📤 Out: {r.delivery_date} · Due: <span style={{ color: r.return_date < today && !r.actual_return ? '#ef4444' : '#666' }}>{r.return_date}</span>
                      {r.actual_return && <><br />📥 Returned: <span style={{ color: '#22c55e' }}>{r.actual_return}</span> by <span style={{ color: '#888' }}>{r.returned_by}</span></>}
                      <br /><span style={{ color: '#3a3a3a' }}>Logged by: {r.created_by}</span>
                      {r.notes && <><br />📝 {r.notes}</>}
                    </div>
                  </div>
                )
        ...
        <Modal title="My Account" onClose={closeModal}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 24, background: '#161616', borderRadius: 12, padding: '14px 16px' }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 11, color: '#555', marginTop: 2 }}>Active user</div>
            </div>
          </div>
          <p style={{ fontSize: 12, color: '#666', marginBottom: 20, lineHeight: 1.6 }}>
            Your actions are recorded in history with your name. To switch users, click Sign Out.
          </p>
          <div style={{ display: 'flex', gap: 8 }}>
            <Btn full variant="danger" onClick={() => { handleLogout(); closeModal() }}>Sign Out / Switch User</Btn>
      ...
      {modal === 'send' && (
        <Modal title="📤 Check Out Tool" onClose={closeModal}>
          <div style={{ background: '#161616', borderRadius: 8, padding: '8px 12px', marginBottom: 16, fontSize: 11, color: '#666' }}>
            Logging as: <strong style={{ color: '#d97706' }}>{currentUser}</strong>
          </div>
          <SelectField label="Available tool *" value={form.toolId || ''} onChange={setF('toolId')}>
            <option value="">Select a tool...</option>
            {tools.filter(t => getToolStatus(t.id) === 'available').map(t => (
              <option key={t.id} value={t.id}>{t.name} ({t.serial})</option>
            ))}
          </SelectField>
          <SelectField label="Destination site *" value={form.siteId || ''} onChange={setF('siteId')}>
            <option value="">Select a site...</option>
            {sites.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
          </SelectField>
          <Input label="Person responsible *" value={form.responsible || ''} onChange={setF('responsible')} placeholder="Who will be using the tool?" />
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            <Input label="Checkout date *" type="date" value={form.deliveryDate || today} onChange={setF('deliveryDate')} />
            <Input label="Expected return *" type="date" value={form.returnDate || ''} onChange={setF('returnDate')} />
          </div>
          <Input label="Notes" value={form.notes || ''} onChange={setF('notes')} placeholder="Optional..." />
          <div style={{ display: 'flex', gap: 8, marginTop: 6 }}>
            <Btn full onClick={handleSend} disabled={saving}>{saving ? 'Saving...' : 'Confirm Checkout'}</Btn>
            <Btn variant="ghost" onClick={closeModal}>✕</Btn>
          </div>
        </Modal>
      )}
      ...
      {modal === 'return' && (
        <Modal title="📥 Return Tool" onClose={closeModal}>
          <div style={{ background: '#161616', borderRadius: 8, padding: '8px 12px', marginBottom: 16, fontSize: 11, color: '#666' }}>
            Logging as: <strong style={{ color: '#d97706' }}>{currentUser}</strong>
          </div>
          <p style={{ color: '#888', fontSize: 14, marginBottom: 22, lineHeight: 1.7 }}>
            Confirm return of<br /><strong style={{ color: '#fff' }}>{form.toolName}</strong><br />
            on <strong style={{ color: '#d97706' }}>{today}</strong>?
          </p>
          <div style={{ display: 'flex', gap: 8 }}>
            <Btn full onClick={() => handleReturn(form.recordId)} disabled={saving}>{saving ? 'Saving...' : 'Confirm Return'}</Btn>
            <Btn variant="ghost" onClick={closeModal}>✕</Btn>
          </div>
        </Modal>
      )}
      ...
      {modal === 'addTool' && (
        <Modal title="🔩 Add New Tool" onClose={closeModal}>
          <Input label="Tool name *" value={form.toolName || ''} onChange={setF('toolName')} placeholder="Ex: DeWalt Drill 800W" />
          <Input label="Serial number / code *" value={form.toolSerial || ''} onChange={setF('toolSerial')} placeholder="Ex: DWT-007" />
          <SelectField label="Category *" value={form.toolCategory || ''} onChange={setF('toolCategory')}>
            <option value="">Select...</option>
            {CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}
          </SelectField>
          <div style={{ display: 'flex', gap: 8, marginTop: 6 }}>
            <Btn full variant="blue" onClick={handleAddTool} disabled={saving}>{saving ? 'Saving...' : 'Add Tool'}</Btn>
            <Btn variant="ghost" onClick={closeModal}>✕</Btn>
          </div>
        </Modal>
      )}
      ...
      {modal === 'addSite' && (
        <Modal title="🏗️ Add New Site" onClose={closeModal}>
          <Input label="Site name *" value={form.siteName || ''} onChange={setF('siteName')} placeholder="Ex: Downtown Renovation" />
          <Input label="Full address *" value={form.siteAddress || ''} onChange={setF('siteAddress')} placeholder="Ex: 500 Main St, Miami, FL" />
          <div style={{ display: 'flex', gap: 8, marginTop: 6 }}>
            <Btn full variant="blue" onClick={handleAddSite} disabled={saving}>{saving ? 'Saving...' : 'Add Site'}</Btn>
            <Btn variant="ghost" onClick={closeModal}>✕</Btn>
          </div>
        </Modal>
      )}
